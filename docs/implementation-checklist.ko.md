...
- [x] client-provided `session_id`는 canonical hyphenated UUID string으로 검증하고 lowercase canonical form으로 정규화한다. UUID v4는 권장하지만 강제하지 않는다.
...
- [x] `stream` 완료 후 별도 `metrics` event로 model/usage와 estimated turn cost를 출력한다. context는 transcript에 있을 때만 출력한다.
