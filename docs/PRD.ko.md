...
새 tmux session을 만들 때는 user prompt를 Claude Code launch argv로 전달한다. 기존 tmux session을 재사용할 때는 prompt paste 전에 Enter를 1회 보내고 2.0초 대기한 뒤 `tmux load-buffer` + `paste-buffer` + `send-keys Enter`로 전달한다. high-level stream/ask의 기존 tmux 입력 경로는 paste 후 기본적으로 Enter를 2회 보낸다. 첫 submit Enter는 paste 후 0.25초, 두 번째 submit Enter는 그 1.0초 뒤에 보낸다. Multi-line prompt는 새 tmux launch 경로에서는 shell ANSI-C `$'...'` quoting의 `\n`으로 보존하고, 기존 tmux 입력 경로에서는 bracketed `paste-buffer -p`를 사용해서 embedded newline이 별도 Enter key가 아니라 하나의 user turn 안의 text로 보존되어야 한다.
...
- If tmux session is active, sends one pre-prompt Enter, waits 2.0s, then sends the prompt to the existing Claude Code process through tmux paste plus one or two Enter submits. The default is two post-paste Enter submits.
