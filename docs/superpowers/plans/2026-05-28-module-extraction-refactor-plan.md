...
| `ctc_state.py` | web session ids, state paths, file locks, bridge state read/write/mutate, turn state records |
...
| `ctc_web.py` | high-level stream/ask/attach/cancel/replay orchestration and active-turn recovery |
...
Lower-level modules must not import `ctc_cli` or `ctc_web`. If a cycle appears between `ctc_state`, `ctc_streaming`, and `ctc_web`, do not solve it by making `ctc_state` depend on runtime streaming, controller, transcript anchoring, or metrics code. Keep `ctc_state` limited to persistence primitives, and put richer active-turn transitions in `ctc_streaming`, `ctc_web`, or a later `ctc_turn_state.py` transition module with explicit dependencies.
...
Do not move runtime/controller/transcript/metrics-aware active-turn transitions in this phase. In particular, keep these outside `ctc_state.py` until streaming and web orchestration boundaries are ready:
...
### Phase 8: Extract Web Session Orchestration And Reap

Move web orchestration after state and streaming APIs are stable:
...
from ctc_compat import *  # or explicit compatibility imports
...
| --- | --- |
| Architecture boundary | Keep `claude_tmux_control.py` as a compatibility facade; use sibling modules; prevent lower modules importing CLI/web orchestration |
