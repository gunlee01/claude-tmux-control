...
| no visible work | high-level `active_turn`이 없거나 `ready` 상태, 또는 오래된 `active_turn`이 남았지만 tmux 화면이 ready |
...
high-level session에서 오래된 `active_turn`이 남아 있으면 `reap`은 먼저 같은 session transcript와 ready tmux 화면으로 해당 turn을 완료 처리할 수 있는지 확인합니다. 실제 reap에서는 가능할 때 turn을 finalize한 뒤 idle 종료 판단을 계속합니다. 완료 처리할 수 없더라도 tmux 화면이 ready이면 idle 기준을 만족한 session을 정리할 수 있습니다. `--dry-run`은 이 확인만 시뮬레이션하고 state를 쓰거나 session을 종료하지 않습니다.

완료 처리할 수 없는 high-level `active_turn`이 남아 있고 tmux 화면이 `ready`가 아니면 오래됐어도 종료하지 않습니다.

`interrupted`는 입력 가능 또는 정리 가능 신호가 아닙니다. 일반 high-level stream timeout은 `Escape`를 보내고 tmux session을 종료한 뒤, cleanup이 성공하면 `active_turn`을 비웁니다. 그래도 `timeout` active turn이 남아 있으면 inspect 또는 `cancel --reset`으로 별도 처리합니다.
...
| stream timeout | `Escape` 전송 성공 시 다음 prompt 허용 |
