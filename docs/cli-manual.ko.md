...
| 대상 | 원본 | 용도 |
| --- | --- | --- |
| 입력 | `tmux load-buffer` + bracketed `paste-buffer -p` + `send-keys Enter` | Claude Code terminal UI에 prompt 전달. embedded newline은 별도 Enter가 아니라 입력 text로 보존 |
...
새 session은 `claude --session-id <uuid> --dangerously-skip-permissions`로 Claude Code를 먼저 시작한 뒤, tmux `load-buffer`/bracketed `paste-buffer -p`/`send-keys Enter`로 prompt를 전송합니다.
...
`paste-buffer -p`는 Claude Code TUI가 bracketed paste mode를 요청한 경우 prompt를 bracketed paste sequence로 감싸 전송합니다. 이 경로는 multi-line prompt 안의 newline이 여러 Enter key로 해석되어 turn이 쪼개지는 것을 막습니다.
