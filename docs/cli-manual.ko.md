...
| 대상 | 원본 | 용도 |
| --- | --- | --- |
| 입력 | `tmux load-buffer` + `paste-buffer` + 짧은 submit delay + `send-keys Enter` | Claude Code terminal UI에 prompt 전달. embedded newline이 있으면 bracketed `paste-buffer -p`로 입력 text 보존 |
