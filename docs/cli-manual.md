...
```bash
TERM=xterm-256color ./claude_tmux_control.py start work --cwd "$PWD"
```

자주 쓰면 alias로 묶습니다.

```bash
alias ctc='TERM=xterm-256color ./claude_tmux_control.py'
```
...
```bash
ACCOUNT_A_TOKEN="$TOKEN" ./claude_tmux_control.py start work-a --cwd "$PWD" --oauth-token-env ACCOUNT_A_TOKEN
ACCOUNT_B_TOKEN="$TOKEN" ./claude_tmux_control.py start work-b --cwd "$PWD" --oauth-token-env ACCOUNT_B_TOKEN
```
...
```bash
./claude_tmux_control.py start work
./claude_tmux_control.py send work "hello"
./claude_tmux_control.py answer work
```

앞으로 외부 client session id를 안정적으로 쓰려면 `ctc-<session-id>` 같은 prefix 규칙을 쓰는 방향입니다.
...
```bash
./claude_tmux_control.py start SESSION [--cwd PATH] [--command COMMAND] [--attach] [--oauth-token-env ENV]
```
...
```bash
./claude_tmux_control.py launch SESSION [--command COMMAND]
```
...
```bash
./claude_tmux_control.py send SESSION "prompt text"
```

stdin에서 읽기:

```bash
printf '긴 프롬프트\n여러 줄\n' | ./claude_tmux_control.py send work
```

입력만 붙여 넣고 Enter를 누르지 않기:

```bash
./claude_tmux_control.py send work --no-enter "draft prompt"
```
...
```bash
./claude_tmux_control.py status SESSION
```
...
```bash
./claude_tmux_control.py wait-ready SESSION --timeout 120
```
...
```bash
./claude_tmux_control.py answer SESSION
```

완료까지 기다린 뒤 출력:

```bash
./claude_tmux_control.py answer SESSION --wait --timeout 120
```

최근 N개 답변 출력:

```bash
./claude_tmux_control.py answer SESSION --tail 3
```
...
```bash
./claude_tmux_control.py turn SESSION
```

계속 갱신:

```bash
./claude_tmux_control.py turn SESSION --follow
```

최근 N개 turn:

```bash
./claude_tmux_control.py turn SESSION --tail 3
```
...
```bash
./claude_tmux_control.py stream SESSION
```
...
```bash
./claude_tmux_control.py events SESSION
```

raw JSON으로 보기:

```bash
./claude_tmux_control.py events SESSION --json --tail 50
```

계속 보기:

```bash
./claude_tmux_control.py events SESSION --follow
```

특정 transcript 파일 직접 보기:

```bash
./claude_tmux_control.py events --transcript ~/.claude/transcripts/session.jsonl --json
```
...
```bash
./claude_tmux_control.py capture SESSION --height 200
```
...
```bash
./claude_tmux_control.py watch SESSION
```
...
```bash
./claude_tmux_control.py follow SESSION --append claude-screen.log
```
...
```bash
./claude_tmux_control.py chat SESSION --cwd "$PWD"
```
...
```bash
./claude_tmux_control.py kill SESSION
```

예:

```bash
./claude_tmux_control.py kill ctc-abc123
```
...
```cron
* * * * * cd /path/to/claude-tmux-control && ./claude_tmux_control.py reap --idle-seconds 1800 --prefix ctc- >> reap.log 2>&1
```
...
```bash
CLAUDE_CODE_OAUTH_TOKEN="$TOKEN" ./claude_tmux_control.py start "ctc-$SESSION_ID" --cwd "$WORKDIR"
```

다음 단계에서는 `ensure <session-id>` 명령으로 이 흐름을 감쌀 계획입니다.

### 사용자 입력 전달

```bash
./claude_tmux_control.py send "ctc-$SESSION_ID" "$USER_PROMPT"
```

동시에 같은 session에 여러 prompt를 보내면 안 됩니다.

session별 lock은 아직 구현 전입니다.

### 진행 상태 polling

```bash
./claude_tmux_control.py status "ctc-$SESSION_ID"
```

또는 완료까지 blocking:

```bash
./claude_tmux_control.py wait-ready "ctc-$SESSION_ID" --timeout 300
```

### 최종 답변 조회

```bash
./claude_tmux_control.py answer "ctc-$SESSION_ID"
```

### tool call 포함 진행 표시

```bash
./claude_tmux_control.py stream "ctc-$SESSION_ID" --timeout 300
```
...
- wrapper는 테스트/디버깅용입니다. 서비스 연동에서는 `send`와 `stream`을 직접 호출해 stdout JSONL을 처리하는 편이 낫습니다.
...
```bash
ACCOUNT_A_TOKEN="..." ./claude_tmux_control.py start ctc-a --oauth-token-env ACCOUNT_A_TOKEN
ACCOUNT_B_TOKEN="..." ./claude_tmux_control.py start ctc-b --oauth-token-env ACCOUNT_B_TOKEN
```
...
```bash
./claude_tmux_control.py start work --command "claude --model opus"
...
```bash
./claude_tmux_control.py events work --tail 20 --json
```
