...
```bash
./claude_tmux_control.py stream --attach --session-id "$SESSION_ID"
```
...
```bash
./claude_tmux_control.py ask --session-id "$SESSION_ID" --cwd "$PROJECT_DIR" "$USER_PROMPT"
```
...
```bash
./claude_tmux_control.py info "$SESSION_ID" --json
```
